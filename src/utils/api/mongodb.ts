...
export class MongoDbClient {
    ...
    filterMongoDbUrl = (str: any) => {
        const newStr: string = typeof str === "string" && str.trim() ? str.trim() : (process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/");
        if (!newStr) {
    ...
    filterMongoDatabase = (str: any) => {
        const newStr: string = typeof str === "string" && str.trim() ? str.trim() : (process.env.MONGODB_NAME || "enter-the-queue");
        if (!newStr) {
