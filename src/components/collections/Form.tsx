...
import { Categories } from '../miscellaneous';
import { NFT_BYTECODE, NFT_ABI, uploadOnIPFSServer } from '../../utils';
...
function CollectionForm(props: any) {
    ...
    const { user, isAuthenticated, loginUserSigner, login, getBalance, web3 }: any = Metamask.useContext();
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            const result = await validateCollectionForm({ ...newFormData, images });
    ...
    const uploadAssetsOnIPFS = async () => {
        try {
            // const isTx = await checkTransaction();
            // if (!isTx) return;
            setModal({
                assets: {
                    isLoader: true,
            ...
            // const nftLogo = "QmZuMv2aZj9rNHJThUT4BHXkhEZ72HnmZv6DY6TekZVabf";
            // const nftFeatureImage = "QmVnwwvW6CNP87BSj8ErJKSGPtmzs8xreqHDAujq35S9Rg";
            // const bannerImage = "QmY1Kxj5GAVTnGP5xM8cdPUr44FDCxNe3bK7ZmjFApX8rT";
            const nftLogo = await uploadOnIPFSServer(assets.image);
            const nftFeatureImage = await uploadOnIPFSServer(assets.feature);
            const bannerImage = await uploadOnIPFSServer(assets.banner);
            const imagesData = {
                image: nftLogo,
                feature: nftFeatureImage,
                banner: bannerImage
            }
            setImages(imagesData);
            const newFormData: any = {
                ...formData,
                image: getIPFSBaseUrl(nftLogo),
                feature: getIPFSBaseUrl(nftFeatureImage),
                banner: getIPFSBaseUrl(bannerImage)
            }
            setFormData(newFormData);
            await deployContract(newFormData, imagesData);
        } catch (error: any) {
    ...
    const deployContract = async (data: any = {}, imageData: any = {}) => {
        try {
            // const isTx = await checkTransaction();
            // if (!isTx) return;
            setModal({
            ...
            const contract = await factory.deploy(
                toCaptalize(data.name),
                data.symbol.toLocaleUpperCase(),
                parseFloat(data.creatorEarning > 0 ? data.creatorEarning : "0"),
            ...
            if (tx) {
                ...
                setFormData({ ...data })
                await saveOnOwnServer(data, imageData);
            } else {
    ...
    const saveOnOwnServer = async (data: any = {}, imageData: any = {}) => {
        try {
            if (!isAuthenticated) {
                await login();
                return;
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files[0]) {
            ...
            setFormData((prev: any) => ({ ...prev, [name]: URL.createObjectURL(files[0]) }));
        }
    }
    ...
    function removeImage(event: any, ref: any) {
        event.preventDefault();
        const { name } = ref.current;
        ref.current.value = "";
        setFormData((prev: any) => ({ ...prev, [name]: "" }));
        setAssets((prev: any) => ({ ...prev, [name]: "" }));
        setImages((prev: any) => ({ ...prev, [name]: "" }));
        setIsLoading(false);
    }
    ...
    return (
        <div className="lg:flex md:flex block flex-row">
            <div className="basis-4/12  profile_edit_left_col px-10 pt-10 lg:pb-0  md:pb-0  pb-10 dark:bg-[#0f1740] bg-[#fafafa]" data-aos="fade-right" data-aos-duration="3000">
                <div className="dark:bg-[#0f1740]  border-rounded-lg md:p-0 p-0">
                    <div className="profile_edit_left_col_imagebg">
                        ...
                        <div className="profile_edit_left_col_image text-center mx-auto mt-0 mb-8 profile_logo_box">
                            {formData.image && <div className="cancle" onClick={(e) => removeImage(e, logoRef)}>X</div>}
                            ...
                            <div className="mb-3 w-100 createpage_left_side_column logobox">
                                ...
                                <input type="file" name='image' id="image" className="hidden" onChange={handleFileChange} accept="image/*" ref={logoRef} />

                            </div>
                        </div>
                        <div className="featured_image_container text-center mx-auto mb-8">
                            {formData.feature && <div onClick={(e) => removeImage(e, featuredRef)} className="cancle">X</div>}
                            <h2 className="dark:text-white text-[#000] text-lg font-semibold aos-init aos-animate mb-3" data-aos="zoom-in" data-aos-duration="3000">
                                Featured image
                            ...
                            <div className="mb-3 w-100 createpage_left_side_column">
                                ...
                                <input type="file" name='feature' id="feature" className="hidden" onChange={handleFileChange} accept="image/*" ref={featuredRef} />

                            </div>
                        </div>
                        <div className="profile_edit_left_col_bannerimg text-center mx-auto mb-8">
                            {formData.banner && <div className="cancle" onClick={(e) => removeImage(e, bannerRef)}>X</div>}
                            ...
                            <div className="mb-3 w-100 createpage_left_side_column">
                                ...
                                <input type="file" name='banner' id="banner" className="hidden" onChange={handleFileChange} accept="image/*" ref={bannerRef} />
            ...
            <div className="basis-8/12  profile_edit_right_col lg:mt-0 md:mt-0 mt-10 dark:bg-[#05092b]  bg-[#fff] border-rounded-lg lg:p-10 md:p-10 p-0" data-aos="fade-left" data-aos-duration="3000">
                <div className="profile_edit_right_col_form_column " >
                    <div className="profile_edit_right_col_form_column_form create_form ">
                        <h3 className='px-0  dark:text-white text-[#000]  text-lg font-semibold '>Create New Collection</h3>
                        <form className="w-full mb-10 mt-8 " data-aos="zoom-in" data-aos-duration="3000">
                                    ...
                                    socialIcons.map((social, key) => {
                                        ...
                                        return (
                                            <div className="flex flex-row lg:mb-2 md:mb-2 mb-0 items-center" key={key}>
                                                ...
                                                <div className="  mb-6 pl-3 md:mb-0 collection_sociallinks">
                            ...
                            <div className="grid grid-cols-1 mt-3 lg:mb-5 md:mb-5 mb-0">
                                <div className=" pl-3 mb-6 md:mb-0 ">
                        ...
                        <ModalFooter className="steps_popup_button">
                            {
                                modal.completed.isComplete
                                    ? <Link href={redirectCollectionUrl} passHref legacyBehavior>
