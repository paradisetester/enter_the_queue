...
const Sell: FC<SellProps> = ({ nft }) => {
    ...
    //             let marketServiceFee = await marketplaceContract.getMarketplaceServiceFee();
    //             marketServiceFee = marketServiceFee.toNumber() || 0;
    //             setServiceFeePer(marketServiceFee);
    //         }
    //     })();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            var price = formData.data?.price || "0";
            var startTime: any = moment();
            var endTime: any = moment();
            let idKeyName = "offerId";
            ...
            const startTimeAuction = startTime.add(5, "minutes").utc().unix();
            const endTimeAuction = endTime.utc().unix();
            ...
            if (listItemTx) {
                ...
                const typeId = event?.args[idKeyName].toString();
                setFormData({
                    ...formData,
                    typeId,
                    transaction: listItemTx,
                })
                await updateListingNftItem(listItemTx, typeId);
                return;
            }
    ...
    const updateListingNftItem = async (tx: any = {}, typeId: string = "") => {
        try {
            setModal({
                ...{
                    listing: {
            ...
            const result = await listNftItem(nft.id, {
                transactions: Object.keys(tx).length ? tx : formData.transaction,
                marketplace: {
                    ...formData.data,
                    type: formData.type,
                    typeId: typeId ? typeId : formData.typeId,
                },
                                                ...
                                                <div className="service_size">
                                                    {
                                                        otherData.serviceFee.eth > 0 ? (
                                                            <span className="text-slate-400 mr-2"><FaEthereum style={{ display: "inline-block", marginTop: "-5px" }} />{parseFloat(otherData.serviceFee.eth.toFixed(4))} </span>
                                                        ) : ""
                                                    }
                                                ...
                                                <div className="service_size">
                                                    {
                                                        otherData.total.dollar > 0 ? (
                                                            <span className="text-slate-400 mr-2">${parseFloat(otherData.total.dollar.toFixed(2))} </span>
                                                        ) : ""
                                                    }
