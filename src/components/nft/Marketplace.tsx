...
import { DVCountDown } from "../miscellaneous";
import { Metamask, MetamaskContextResponse } from "../../context";
import { CustomModal, ModalContent, ModalFooter, ModalHeader } from "..//miscellaneous/modal";
...
const BuyerMarketPlaceComponent: FC<MarketplaceComponentProps> = ({ nft, useStates, useModalOpen, setError, useUpdate }) => {
    ...
    const handlePlaceAuctionBid = async (event: any) => {
        ...
        try {
            ...
            let message = "";
            if (bidPrice <= 0) message = "Bid price must be greater than zero!";
            if (bidPrice <= maxBidPrice) message = `Bid price must be greater than ${maxBidPrice}`;
            if (!message) {
                setOpen(true);
                await placeABid();
            }
    ...
    const placeABid = async () => {
        try {
            ...
            if (!errorData?.slug) {
                errorData.slug = "auction";
            }
            throw new Error(JSON.stringify(errorData));
        }
    }

    const placeABidAPI = async (tx: any = {}) => {
        try {
            if (!isAuthenticated) {
                login();
                return;
    ...
    return <>
                    ...
                    <>
                        {(() => {
                            switch (marketplace.action) {
                                ...
                                case 'open_for_bids':
                                    return <div className="open-for-bids-section dark:text-white py-3">
                                        ...
                                        <div className="grid grid-cols-2 gap-2" >
                                            ...
                                            <div className="myprofile_onsale_column_box_creator_owned">
                                                <button
                                ...
                                case 'timed_auction':
                                    return (
                                        <div className="timed-auction-section">
                                            ...
                                            <div className=" my-5 nftdetail_bids  singlepage_owner_list flex ">
                                                <div className="nftdetail_bids_col">
                                                    <div className="ml-2 items-center">
                                                        <h6 className=" dark:text-[#fff] text-[#000] capitalize">Higher Bid:</h6>
                                                ...
                                                <div className="nftdetail_bids_col">
                                                    <div className="ml-2 items-center">
                                                        <h6 className=" dark:text-[#fff] text-[#000] capitalize">Min Bid:</h6>
                                                        <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                ...
                                                <div className="nftdetail_bids_col">
                                                    <div className="ml-2 items-center">
                                                        <h6 className=" dark:text-[#fff] text-[#000] capitalize">Last Bidder:</h6>
                                                        <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                            {
                                                                currentBidData?.winner?.id ? (
                                                                    <Link href={`creator/${currentBidData.winner.id}`} className="mr-1">{currentBidData?.winner?.username || ""}</Link>
                                                                ) : (
        ...
        >
            <ModalHeader onClose={() => setOpen(false)}>
                <span className="font-bold text-black">Follow steps</span>
            </ModalHeader>
                                        ...
                                        return (
                                            <div key={key} className={`${key > 0 ? 'mt-3' : ""}`}>
                                                <Grid container spacing={2}>
                                                    ...
                                                    <Grid item xs>
                                                        <h1 className="font-bold text-[#000] dark:text-[#000]">{toCaptalize(defaultStates.others[value].title)}</h1>
        ...
        >
            <ModalHeader onClose={() => resetModal()}>
                <span className="font-bold text-black">{modalData.title}</span>
            </ModalHeader>
            <ModalContent className='popup_form_title'>
                {
...
const OwnerMarketPlaceComponent: FC<MarketplaceComponentProps> = ({ nft, useStates, useModalOpen, setError, useUpdate }) => {
    ...
    const acceptOffer = async (offerId = "", _offerer: UserWalletAddress = "") => {
        try {
            ...
            const marketPlaceTransaction = await marketplaceContract.acceptOfferForItem(
                _offerer,
                parseInt(marketplace.actionId),
                options
            );
    ...
    const transferToAuctionWinner = async () => {
        try {
            ...
            const marketPlaceTransaction = await marketplaceContract.transferAuctionItem(
                parseInt(itemId)
            );
            const tx = await marketPlaceTransaction.wait();
            if (tx) {
                    ...
                    <>
                        {(() => {
                            switch (marketplace.action) {
                                ...
                                case 'timed_auction':
                                    return (
                                        <div className="timed-auction-section timed-auction-owner-section dark:text-white py-3">
                                            <div className="">
                                                <div className="nftdetail_countdown ">
                                                    <DVCountDown
                                                ...
                                                <div className=" my-5 nftdetail_bids  singlepage_owner_list flex" style={{ border: 'none' }}>
                                                    <div className="nftdetail_bids_col">
                                                        <div className="ml-2 items-center">
                                                            <h6 className=" dark:text-[#fff] text-[#000] capitalize">Higher Bid:</h6>
                                                            <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#000] text-[#707a83] text-xs flex">
                                                    ...
                                                    <div className="nftdetail_bids_col">
                                                        <div className="ml-2 items-center">
                                                            <h6 className=" dark:text-[#fff] text-[#000] capitalize">Winner:</h6>
                                                            <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                                {
                                                                    currentAuction.winner?.id ? (
                                                                        <Link href={`creator/${currentAuction.winner.id}`} className="mr-1">{currentAuction.winner.username || ""}</Link>
                                                                    ) : (
                                                                        <span>No Bid Yet</span>
                                                                    )
                                                                }
                                                                        ...
                                                                        currentAuction.auctionHistories.map((auctionHistory: any, key: number) => {
                                                                            ...
                                                                            return (
                                                                                <tr key={key}>
                                                                                    ...
                                                                                    <td className="border-b border-slate-200 dark:border-slate-600 py-4 px-2 text-slate-500 dark:text-slate-400">
                                                                                        <Link href={url} className="text-[#571a81]" legacyBehavior>{createdBy}</Link>
                                                                                    </td>
