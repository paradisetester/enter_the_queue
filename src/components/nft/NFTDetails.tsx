...
import { IsJsonString, getTransactionOptions, formatSolidityError, subString, trimString, getNftMarketContract } from "../../helpers";
...
const NFTDetail: FC<DetailProps> = ({ details, useUpdate }) => {
  ...
  const {
    ...
    metadata, properties, transaction, category, isLiked, marketplace, itemId
  } = details;
  ...
  const cancelInBlockchainSale = async () => {
    try {
      ...
      const typeId = marketplace.actionId || 0;
    ...
    <div className="dark:bg-[#09080d] bg-[#fff]">
      <section className="productdetail_section lg:px-14 sm:px-3 lg:pb-0 md:pb-0 pb-0 pt-20">
        <div className="sale-section remove-from_bids" style={saleSectionStyle}>
          {(() => {
            if (isAuthenticated && currentUserId === ownedBy.id && (user.role === "ARTIST" && user.isApproved || user.role === "ADMIN")) {
              const action = marketplace?.action || "";
              return action
                ? <Button onClick={handleRemoveFromSale} color="primary" variant="contained" size="large">Remove from Sale</Button>
                : (
                  <Link href={`/discover/${id}/sell`} passHref legacyBehavior>
                    <Button color="primary" autoFocus variant="outlined" size="large">Put On Sale</Button>
                  </Link>
                );
            }
            return '';
            ...
            >
              <div className="m-1 p-2 productdetail_section_image rounded-lg">
                <DVAsset
                  ...
                  className={asset.type === "video" ? "form-label inline-block mb-2 text-gray-700" : "rounded mt-4"}
                />
              </div>
              ...
              <div className="productdetail_section__col_content_box m-1  p-2]">
                <div className="productdetail_section_col_content_box py-3 px-3">
                  ...
                  <div className="mt-5 singleproduct_viewproof">
                    <div className="grid grid-cols-2 gap-2 nftdetail_category" >
                      <div className="view-proof-authenticity-section dark:text-[#fff] text-[#000] text-xs font-bold flex">
                        <p className="grid grid-cols-12 gap-2">
                          <span className="col-span-1">
                            <GiToken style={{ display: "inline-flex" }} />
                          </span>
                      ...
                      <div className="view-proof-authenticity-section dark:text-[#fff] text-[#000] text-xs font-bold flex">
                        <p className="grid grid-cols-12 gap-2">
                          <span className="col-span-1">
                            <BsCollectionFill style={{ display: "inline-flex" }} />
                          </span>
                          <span className="col-span-5 text-black dark:text-white">Collection : </span>
                          <span className="col-span-6 text-[#571a81]"><Link href={`/collections/${collection.id}`} passHref legacyBehavior>{trimString(collection.name)}</Link></span>
                        </p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mt-2" >
                      <div className="view-proof-authenticity-section dark:text-[#fff] text-[#000] text-xs font-bold flex">
                        <p className="grid grid-cols-12 gap-2">
                          <span className="col-span-1">
                            <BiCategory style={{ display: "inline-flex" }} />
                          </span>
