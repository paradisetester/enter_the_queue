...
export const artist = object().shape({
    socialLinks: object({
    ...
    email: string().trim().required("Email must be required"),
    username: string().trim().required("Username must be required"),
    name: string().trim().required("Name must be required"),
});

export const validateArtistForm = async (formData: FormProps) => {
    const result = await artist.validate(formData)
        .then((value) => {
            return {
                status: true,
                errors: [],
